...
model.hierarchical <- MCMChlogit(fixed = contraceptive_use ~ living.children, random = ~ living.children, group = "district", data = dataset_2, r = 2, R = diag(c(1, 0.1)))
...
ggplot(coefficient_graph_data, aes(districts)) + geom_point(aes(y = pooled, colour = "Pooled Coefficients")) + geom_point(aes(y = unpooled, colour = "Unpooled Coefficients")) + geom_point(aes(y = hierarchical, colour = "Bayesian Hierarchical Coefficients")) + ylab("Coefficient Value") + xlab("District") + ggtitle("Model Coefficient by District")
```
