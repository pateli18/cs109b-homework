...
log_graph_data <- as.data.frame(table(dataset2_test$district))
log_graph_data <- rename(log_graph_data, c("Var1" = "district", "Freq" = "count"))
log_graph_data$individual_model_acc <- individual_log_acc
acc.individual.log <- sum(log_graph_data$count * log_graph_data$individual_model_acc)/ sum(log_graph_data$count)
```
...
print(sprintf("Overall Model Accuracy: %.2f", acc.general.log))

```

The overall model performs better than the individual models when looking across the entire dataset, however there is room from improvement in the overall model, so if we could incorporate some of the components of the local models into the overall model that may be beneficial.

```{r mcmc_hierarchical_model}
model.hierarchical <- MCMChlogit(fixed = contraceptive_use ~ urban + living.children + age_mean, random = ~ urban + living.children + age_mean, group = "district", data = dataset2_train, r = 4, R = diag(c(1, 0.5, 0.5, 0.5)))
...
log_graph_data$mcmc_individual_acc <- mcmc_individual_acc
acc.individual.mcmc <- sum(log_graph_data$count * log_graph_data$mcmc_individual_acc)/ sum(log_graph_data$count)
