...
dataset2_train[, c(2,3,5)] <- sapply(dataset2_train[, c(2,3,5)], as.integer)
dataset2_test <- read.table("dataset_2_test.txt",header = TRUE, sep = ",")
dataset2_test[, c(2,3,5)] <- sapply(dataset2_test[, c(2,3,5)], as.integer)
```
...
acc.general.log <- sum(diag(table.log))/ sum(table.log)
acc.general.log <- rep(acc.general.log, length(districts))

# Measure accuracy of general regression on each individual district
general_log_acc = rep(0., length(districts))
for (i in 1:length(districts)) {
  test_set <- dataset2_test[dataset2_test$district == districts[i], ]
  pred.log <- predict(model.general.log, newdata = test_set, type = "response")
  table.log <- table((pred.log>.5), test_set$contraceptive_use)
  general_log_acc[i] <- sum(diag(table.log))/ sum(table.log)
}
```

```{r plot_log_models}
# Plot the accuracy of the various models by district
log_graph_data <- data.frame(district = districts, individual_model_acc = individual_log_acc, general_model_overall_acc = acc.general.log, general_model_individual_acc = general_log_acc)

ggplot(log_graph_data, aes(x = district, y = accuracy)) + 
  geom_line(aes(y = individual_model_acc, colour = "Individual Model Accuracy"), size = 1) + 
  geom_line(aes(y = general_model_overall_acc, colour = "General Model Overall Accuracy"), size = 1) +
  geom_line(aes(y = general_model_individual_acc, colour = "General Model Individual Accuracy"), size = 1) + ggtitle("Log Model Performance")
```

The individually fit models outperform the generally fit model for some districts, whereas for others the opposite is true. Therefore, if we could combine both models through a hierarchical model, we may acheive superior performance.